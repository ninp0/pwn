    ...
    module CLI
      public_class_method def self.parse(opts = {})
        ...
        parser = OptionParser.new do |options|
          options.banner = 'Usage: pwn-ai [--analyze PATH | --replay ID | --rerun ID] [--ai PROMPT]'
        ...
        raise OptionParser::InvalidArgument, 'choose only one of --analyze, --replay, --rerun' if %i[analyze replay rerun].count { |key| result.key?(key) } > 1
